        ...
        <div id="body" class="body" >
            ...
            <div class="list" :class="{isWeiXin:isWeiXin}">
                ...
                <div v-for="(item,index) in output" >
                    ...
                    <div class="item">
                        <div class="top flexbox" @click="goUrl (item)">
                            ...
                            <div class="right flex flexbox flex-right">
                                ...
                                -->
                                <div>
                                    <span  v-show="item.sts<20">待出单</span>
                                    <span  v-show="item.sts==20">保障中</span>
                                    <span  v-show="item.sts==30">已退保</span>
                                    <span  v-show="item.sts==40">已过期</span>
                                </div>
                                <div v-show="item.instalmentSts != 00">
                                    <span>分期</span>
                                </div>
                            </div>
                        </div>
                        <div class="center" :class="{past:item.sts>20}" @click="goUrl (item)">
                            <div>
                                <div>
                                    车牌号：
                        ...
                        <div class="bottom flexbox" :class="{past:item.sts>20}">
                            <div @click="goUrl (item)">
                                保单金额：
                                <span v-text="(item.payAmount/100).toFixed(2)" :class="{past:item.sts>20}"></span>
                                元
                            </div>
                            <div class="flex flexbox justify-content flex-right " v-show="item.instalmentSts != 10 && item.instalmentSts != 00" @click="goUrl(item, 1)" :class="{past:item.sts>20}">
                                <span v-show="item.repaymentStatus == 11" :class="{past:item.sts>20}">待还款</span>
                                <span v-show="item.repaymentStatus == 12" :class="{past:item.sts>20}">已结清</span>
                                <span v-show="item.repaymentStatus == 13" :class="{past:item.sts>20}">已逾期</span>
                                <i v-show="item.repaymentStatus == 11 || item.repaymentStatus == 12 || item.repaymentStatus == 13" class="iconfont">&#xe61d;</i>
                            </div>
                        </div>
                        <!--item-->
                    </div>  
    ...
    methods: {
        getData1 (me) {
            ...
            var data = {
                ...
                policySts: '30,40', // 状态 0：全部20：已承保； 30：已退保； 40: 已过期
       ...
       // 去保单详情
        goUrl (item, number) {
            if (number === 1) {
                window.location.href = 'FQcarStagesDetil.html?tradeId=' + item.tradeId;
            } else {
        ...
        alertShow (value) {
            this.showLayer(value, {
                ...
                btns: ['我知道啦'], // 必填
                yes: () => { // 选填
                    storage.set('prevPage', './FQpolicyHistory.html');
